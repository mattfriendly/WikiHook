...
const char *LOCK_FILE = "/tmp/wikihook.lock";
...
void clientMode() {
    ...
    while ((row = mysql_fetch_row(res)) != NULL) {
        ...
        std::string cmd = "curl -X POST -H \"Content-Type: application/json\" ";
        cmd += "-d '" + payload + "' ";
        cmd += getenv("CLIENT_URL");

        writeToLog("Executing: " + cmd);

        // Execute the curl command
        int result = system(cmd.c_str());
        if (result != 0) {
            writeToLog("Curl command failed.");
        } else {
            writeToLog("Curl command succeeded.");
        }
    }
...
int main() {
    ...
    if (access(LOCK_FILE, F_OK) == 0) {
        std::cerr << "Error: WikiHook is already running." << std::endl;
        exit(EXIT_FAILURE);
    }
